...
def aggregate_activity(history, n_days, now_datetime, users):
    ...
    for message in history:
        try:
            user = message['user']
            day_offset = get_day_offset_for_ts(
                float(message['ts']), now_datetime)
...
def make_activity_report(active_users_dict, users, n_days, mode=ReportMode.LITE):
    ...
    for uid in sorted(active_users_dict, key=lambda k: (sum(active_users_dict[k].values()), -min(active_users_dict[k], default=99))):
        ...
        count_strs = "".join(["%2s " % active_users_dict[uid].get(
            day_offset, 0) for day_offset in range(n_days)])
        if mode == ReportMode.LITE:
...
def make_calls_activity_report(active_users_dict, calls_history, users, initiators_only=False):
    ...
    for uid in sorted(active_users_dict, key=lambda k: (-len([c for c in calls_history if c['user'] == k or (k in c['participants'] and not initiators_only)]))):
        ...
        res.append("%s %3d calls  (%s minutes)" %
                   (nameCol, totalCalls, durations))
...
def parse_command_line():
    ...
    parser.add_argument('-d', '--num_days', default=5, type=int,
                        help='number of days over which to look back')
    parser.add_argument('-l', '--log_raw_json', action='store_true',
                        help='log raw json response to file: sensitive/channels.history.json')
    parser.add_argument('-g', '--graph_calls', action='store_true',
                        help='graph calls to dot file: sensitive/calls.dot')
    parser.add_argument('-f', '--use_fake_data', action='store_true',
                        help='use fake data instead of getting from slack API')
...
def read_env_vars():
    ...
    input_channel = env_var_to_dict(
        "SLACK_INPUT_CHANNEL", ['channel_id', 'channel_name'])
    output_channel = env_var_to_dict("SLACK_OUTPUT_CHANNEL", [
                                     'channel_id', 'channel_name'])
    users_dict = env_var_to_dict_of_dicts(
        "SLACK_USERS", ["user_id", "user_name", "real_name"], "user_id", "/")
...
def run():
    ...
    report1 = make_activity_report(
        active_users_dict, users, n_days, mode=ReportMode.LITE)
    report2 = make_activity_report(
        active_users_dict, users, n_days, mode=ReportMode.FULL)
    calls_summary_report = make_call_summary_report(calls_list, users)
    if should_graph_calls:
        graphviz_calls_summary_report = make_call_summary_report(
            calls_list, users, for_graphviz=True)
        with open('sensitive/calls.dot', 'w') as f:
            f.write(graphviz_calls_summary_report)

    calls_report = make_calls_activity_report(
        active_users_dict, calls_list, users)
    call_starters_report = make_calls_activity_report(
        active_users_dict, calls_list, users, initiators_only=True)

    # Assemble the full_message
    full_message = '\n'.join(['```', introduction, report1, report2,
                              calls_summary_report, calls_report, call_starters_report, '```'])
